...
add_phone_number() {
  read -p "Enter a phone number to add (or type 'done' to finish): " phone_number
  while [[ "$phone_number" != "done" ]]; do
    phone_numbers+=("$phone_number")
    echo "Added $phone_number. Enter another number or type 'done' to finish."
    read phone_number
  done
}

# Call the function to add phone numbers
add_phone_number
